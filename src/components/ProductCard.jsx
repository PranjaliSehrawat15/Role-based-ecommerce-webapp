...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="group bg-[#0f1433] rounded-2xl overflow-hidden border border-[#3f4663] shadow-lg hover:shadow-[0_0_40px_rgba(0,212,255,0.25)] transition-all duration-300 flex flex-col">
      ...
      <div className="relative aspect-square overflow-hidden">
        ...
        {/* CATEGORY BADGE */}
        {product.category && (
          <span className="absolute top-3 left-3 bg-gradient-to-r from-purple-600 to-cyan-500 text-white text-xs font-semibold px-3 py-1 rounded-full shadow">
            {product.category}
          </span>
        )}
        ...
        {/* STOCK BADGE */}
        {!outOfStock && product.stock < 5 && (
          <span className="absolute top-3 right-3 bg-yellow-500 text-black text-xs font-bold px-3 py-1 rounded-full animate-pulse">
            Only {product.stock} left
          </span>
        )}
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <h3
          onClick={() => navigate(`/product/${product.id}`)}
          className="text-white font-semibold text-sm leading-5 line-clamp-2 cursor-pointer hover:text-cyan-400 transition"
        >
          {product.name}
        </h3>
        ...
        {/* BUTTONS */}
        <div className="mt-auto pt-4 flex gap-2 border-t border-[#3f4663]">

          <button
            onClick={() => navigate(`/product/${product.id}`)}
            className="flex-1 py-2 rounded-lg border border-cyan-400 text-cyan-400 text-sm font-semibold hover:bg-cyan-400/10 transition"
          >
            View
          </button>
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-semibold transition ${
              outOfStock
                ? "bg-[#3f4663] text-gray-400 cursor-not-allowed"
                : "bg-gradient-to-r from-purple-600 to-cyan-500 text-white hover:shadow-lg active:scale-95"
            }`}
          >
